...
4. Install all backend python packages, including developer test tools (pytest, black, ruff, mypy).
5. Run the environment verification checklist (`scripts/verify_env.py`).
...
*   **Start the complete stack**:
    ...
    make start
    ```
    *(Under the hood: runs `docker compose up --build -d`)*
...
*   **Stop and wipe volumes**:
    ...
    make stop
    ```
    *(Under the hood: runs `docker compose down -v`)*
...
*   **Linting & Typing**: Validate syntax rules, typing parameters, and ESLint configs:
    ...
    make lint
    ```
*   **Testing**: Execute frontend and backend pytest suites:
    ```bash
    make test
    ```
